 ...
 * @extends {ItemSheetV2}
 */
export class OutsideTheWireItemSheet extends api.HandlebarsApplicationMixin(sheets.ItemSheetV2) {
  constructor(options = {}) {
    super(options);
  }
  ...
  static DEFAULT_OPTIONS = {
    ...
    actions: {
      ...
      convertToSof: this._convertToSof,
    },
    form: {
      submitOnChange: true,
    },
      ...
      controls: [
        {
          ...
          label: "OUTSIDE_THE_WIRE.Item.Specialty.ConvertToSof",
          action: "convertToSof",
          visible: OutsideTheWireItemSheet.#canConvertToSof
        }]
    }
  };
  ...
  static PARTS = {
    header: {
      template: 'systems/outside-the-wire/templates/item/header.hbs',
    },
    tabs: {
      template: 'templates/generic/tab-navigation.hbs',
    },
    description: {
      template: 'systems/outside-the-wire/templates/item/description.hbs',
    },
    specialty: {
      template: 'systems/outside-the-wire/templates/item/configuration-parts/specialty.hbs',
    },
    deployment: {
      template: 'systems/outside-the-wire/templates/item/configuration-parts/deployment.hbs',
    },
    deploymentDetail: {
      template: 'systems/outside-the-wire/templates/item/configuration-parts/deployment-detail.hbs',
    },
    equipment: {
      template: 'systems/outside-the-wire/templates/item/configuration-parts/equipment.hbs',
    },
    weapon: {
      template: 'systems/outside-the-wire/templates/item/configuration-parts/weapon.hbs',
    },
    explosive: {
      template: 'systems/outside-the-wire/templates/item/configuration-parts/explosive.hbs',
    },
    bonuses: {
      template: 'systems/outside-the-wire/templates/item/bonuses.hbs',
      templates: [
  ...
  _configureRenderOptions(options) {
    ...
    if (Object.keys(this.constructor.PARTS).indexOf(this.document.type) > -1)
      options.parts.push(this.document.type);

    options.parts.push('bonuses');
  ...
  static async _deleteAutomation(event, target) {
    ...
    const type = game.i18n.localize("OUTSIDE_THE_WIRE.Automation.Type.automation");
  ...
  static #canConvertToSof() {
    return this.document.type == "specialty";
  }

  static async _convertToSof() {
    return this.document.system.convertToSof();
  }
  ...
  async _onDropItem(event, data) {
    ...
    await this.document.system.updateBasedOnItem?.(targetItem);
  ...
  async _onDropMacro(event, data) {
    ...
    if (!automation || !event.target.name || !macroFields.includes(event.target.name.split('.')[3])) {
      ui.notifications.warn(game.i18n.localize("OUTSIDE_THE_WIRE.Automation.FIELDS.macro.error"));
      return;
    }
